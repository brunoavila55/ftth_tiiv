...
- **CI** (`.github/workflows/ci.yml`): `permissions: contents: read`; actions fixadas por SHA (comentário com a versão; o Dependabot atualiza); jobs: backend (ruff/mypy/contrato/migrações/pytest/restore drill), frontend (lint/typecheck/vitest/build), **security** (pip-audit, `pnpm audit --audit-level high`, gitleaks) e **docker-build** (`docker compose build` + Trivy CRITICAL/HIGH nas imagens). O workflow `codeql.yml` foi removido: o repositório é privado e não tem GitHub Advanced Security, então o CodeQL rodava mas o upload do SARIF sempre falhava ("Code scanning is not enabled for this repository"). Se o repositório se tornar público ou o Advanced Security for habilitado, pode-se recriar o workflow (modelo em `docs/security-audit/resolucao.md`, item CodeQL).

...
| **B03** | Sessões, usuários e permissões | `tests/integration/test_auth.py`<br>`tests/integration/test_users.py`<br>`tests/unit/test_cli_bootstrap.py` | ✅ PASS | Login/logout seguro, proteção CSRF dupla, mitigação de enumeração por tempo, rate limiting PostgreSQL, proteção do último admin e CLI de bootstrap. |
...
| **B08** | Rastreamento óptico ponta a ponta | `tests/integration/test_optical_path_tracing.py` | ✅ PASS | Travessia downstream e upstream, semântica de splitters balanceados e assimétricos, detecção de pontas abertas e ciclos inválidos. |
...
| **F17** | Assistentes de import/export e usuários | `tests/imports-exports-wizard.test.tsx`<br>`tests/settings-users.test.tsx` | ✅ PASS | Wizard de exportação GeoJSON/CSV, importação com preview de conflitos e gestão de usuários. |
...
| **F19** | Testes integrados e qualidade frontend | Todas as suítes (19 arquivos, 166 testes) | ✅ PASS | Execução limpa e determinística do Vitest sem regressões. |

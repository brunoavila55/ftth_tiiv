...
2. ~~`S3Storage`: URLs pré-assinadas para download direto do bucket (hoje o backend sempre faz proxy do stream; evita carga no backend em arquivos grandes).~~ **Feito.** `S3Storage.presigned_url()` (`backend/app/core/storage_backend.py`) gera a URL via `boto3.generate_presigned_url` (só assina localmente, sem chamada de rede); os três endpoints de download (`attachments.py`, `imports_exports.py`) redirecionam (307) para ela quando disponível, e continuam com `StreamingResponse` quando não. **Desligado por padrão** (`S3_PUBLIC_ENDPOINT_URL` vazio): o endpoint do MinIO em `compose.s3.yaml` (`http://minio:9000`) só é alcançável dentro da rede Docker, nunca pelo navegador — habilitar exige expor o MinIO publicamente (Caddy com TLS próprio, decisão de instalação) e configurar essa variável com o endpoint público. Testado com `moto` (URL assinada aponta para o host público configurado, não para o interno).
...
4. ~~Backup incluir o bucket S3 (hoje `scripts/backup.py`/`restore.py` só cobrem o volume local `STORAGE_PATH`) — necessário antes de usar `STORAGE_BACKEND=s3` em produção sem depender só da durabilidade própria do MinIO/S3.~~ **Feito.** `create_backup`/`restore_backup` (`backend/app/core/backup_restore.py`) usam `StorageBackend.list()`/`open_read()`/`save_stream()` em vez de andar pelo filesystem quando `STORAGE_BACKEND=s3` e nenhum `--storage-path` é passado explicitamente — baixam/restauram o bucket inteiro, dentro do mesmo pacote assinado e com o mesmo checksum SHA256 dos anexos. Path traversal no nome dos membros do `attachments.tar.gz` é rejeitado antes de gravar qualquer objeto no bucket (mesma proteção do caminho local). O serviço `backup` do compose ganhou as variáveis `STORAGE_BACKEND`/`S3_*`. Testado com `moto` (backup, exclusão simulada dos objetos e restauração).
5. `PostgresRateLimiter` atrás da interface `RateLimiter`.
6. Guia de HA do PostgreSQL e teste de failover no runbook.

## Consequências

- Escalar no mesmo host já é suportado e testado; escalar entre hosts com storage S3/MinIO também (item 1), com backup do bucket (item 4) e downloads pré-assinados opcionais (item 2).
